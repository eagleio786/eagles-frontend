...
const Levelx1 = () => {
  const initialReferredUsers = 18;
  const [activeLevel, setActiveLevel] = useState(4); // Track active level
  const levels = [
  ...
  const handleActivateNextLevel = () => {
    setActiveLevel((prevLevel) => Math.min(prevLevel + 1, 12)); // Level kabhi decrease nahi hoga
  };
